...
get_same_N_rand_comparison_peaks_names <- function(res, N){
  ...
  ### 3.0 Get all DARs that are significant in AllCells ------------------------
...
All_DARs_AllCells_ALS <- unique(
  unlist(
    lapply(
      DESeq_Results_8SVs_ALS[ind_AllCells_Case_Comparisons], 
...
All_DARs_AllCells_ALSFTD <- unique(
  unlist(
    lapply(
      DESeq_Results_8SVs_ALSFTD[ind_AllCells_Case_Comparisons], 
      FUN=get_sign_peaks_names
    )
  ) 
)
...
i <- 0
All_DARs_AllCells_R2 <- unique(
  unlist(
    lapply(
      DESeq_Results_8SVs_ALS[ind_AllCells_Rand_Comparisons], 
      FUN=function(x){
        i <<- i + 1
        return(
...
i <- 0
All_DARs_AllCells_R3 <- unique(
  unlist(
    lapply(
      DESeq_Results_8SVs_ALSFTD[ind_AllCells_Rand_Comparisons], 
      FUN=function(x){
        i <<- i + 1
        return(
          get_same_N_rand_comparison_peaks_names(
...
table(All_DARs_AllCells_ALS %in% All_DARs_AllCells_ALSFTD)
table(All_DARs_AllCells_ALSFTD %in% All_DARs_AllCells_ALS)

table(All_DARs_AllCells_R2 %in% All_DARs_AllCells_R3)
table(All_DARs_AllCells_R3 %in% All_DARs_AllCells_R2)
...
ALS_ALSFTD_AllCells_Signature <- data.frame(
  Peak = sort(
    unique(c(
      All_DARs_AllCells_ALS, 
      All_DARs_AllCells_ALSFTD
    ))
  )
)

table(ALS_ALSFTD_AllCells_Signature$Peak %in% ATAC_Peak_Links$peak)
table(unique(ATAC_Peak_Links$peak) %in% ALS_ALSFTD_AllCells_Signature$Peak)

table(ALS_ALSFTD_AllCells_Signature$Peak %in% Peaks$ID2)


ALS_ALSFTD_AllCells_Signature$ID2 <- ALS_ALSFTD_AllCells_Signature$Peak
ALS_ALSFTD_AllCells_Signature$ID <- Peaks$ID[match(ALS_ALSFTD_AllCells_Signature$Peak, Peaks$ID2)]


ALS_ALSFTD_AllCells_Signature$Signif_ALS <- ALS_ALSFTD_AllCells_Signature$Peak %in% All_DARs_AllCells_ALS
ALS_ALSFTD_AllCells_Signature$Signif_ALSFTD <- ALS_ALSFTD_AllCells_Signature$Peak %in% All_DARs_AllCells_ALSFTD
...
any(duplicated(ALS_ALSFTD_AllCells_Signature$Peak))
...
ALS_ALSFTD_AllCells_Rand_Signature <- data.frame(
  Peak = sort(
    unique(c(
      All_DARs_AllCells_R2, 
      All_DARs_AllCells_R3
    ))
  )
)

table(ALS_ALSFTD_AllCells_Rand_Signature$Peak %in% ATAC_Peak_Links$peak)
table(unique(ATAC_Peak_Links$peak) %in% ALS_ALSFTD_AllCells_Rand_Signature$Peak)

table(ALS_ALSFTD_AllCells_Rand_Signature$Peak %in% Peaks$ID2)


ALS_ALSFTD_AllCells_Rand_Signature$ID2 <- ALS_ALSFTD_AllCells_Rand_Signature$Peak
ALS_ALSFTD_AllCells_Rand_Signature$ID <- Peaks$ID[match(ALS_ALSFTD_AllCells_Rand_Signature$Peak, Peaks$ID2)]


ALS_ALSFTD_AllCells_Rand_Signature$Signif_R2 <- ALS_ALSFTD_AllCells_Rand_Signature$Peak %in% All_DARs_AllCells_R2
ALS_ALSFTD_AllCells_Rand_Signature$Signif_R3 <- ALS_ALSFTD_AllCells_Rand_Signature$Peak %in% All_DARs_AllCells_R3
...
any(duplicated(ALS_ALSFTD_AllCells_Rand_Signature$Peak))
...
qsave(
  ...
  paste0(
    ...
    "Signature_ATAC_ALS_ALSFTD_AllCells", 
    ".qrds"
  ), 
  nthr=nthr
)
